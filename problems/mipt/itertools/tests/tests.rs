...
fn lazy_cycle_simple() {
    ...
    for i in 0..1000 {
        ...
        assert_eq!(advanced_count.get(), 3 + (i + 1));
...
fn lazy_cycle_skip() {
    ...
    let to_skip = [3, 4, 5, 6, 7, 8, 9].into_iter().collect();
...
fn tee_simple() {
    ...
    let (mut first, mut second) = vec.into_iter().tee();
...
fn tee_cloning() {
    ...
    let (mut first, mut second) = elems.into_iter().tee();

    for i in 0..5 {
        let item1 = first.next().unwrap();
        let item2 = second.next().unwrap();

        assert_eq!(item1.payload, i);
        assert_eq!(item2.payload, i);
        
        assert_eq!(item1.cloned_count(), 2);
        assert_eq!(item2.cloned_count(), 2);
    }
...
fn tee_empty() {
    ...
    for i in 0..5 {
        assert!(first.next().is_none());
        assert_eq!(advanced_count.get(), 2*i+1);
